...
from .utils.jwt_keys import get_rsa_private_key
...
def jwt_token_async_view(request):
    ...
    try:
        ...
        # Generate JWT token using RSA private key from environment
        # WARNING: This endpoint should be disabled or restricted in production
        # Tokens should be generated by portbro.com, not locally
        rsa_private_key = get_rsa_private_key()
        
        if not rsa_private_key:
            return JsonResponse({
                'success': False,
                'error': 'RSA private key not configured. Token generation is disabled.'
            }, status=500)
